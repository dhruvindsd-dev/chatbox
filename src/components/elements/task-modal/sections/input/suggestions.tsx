...
import { AnimatePresence, motion, MotionConfig, MotionProps } from "motion/react"
...
const Suggestions = ({ title, setTag, tags: selectedTags }: Props) => {
	...
	useEffect(() => {
		...
		const filteredTags = TAGS_DATA.filter((tag) => (title ? tag.label.toLowerCase().includes(title.toLowerCase()) : false)).filter((tag) =>
	...
	return (
		<MotionConfig transition={{ duration: 0.2, ease: "easeOut" }}>
			<motion.div initial={false} animate={{ height: visible ? "auto" : 0, opacity: visible ? 1 : 0 }} className="overflow-hidden">
