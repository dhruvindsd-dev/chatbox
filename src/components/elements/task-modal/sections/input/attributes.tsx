...
import { ASSIGN_DATA, PRIORITY_DATA, PROJECTS_DATA, STATUS_DATA, TAGS_DATA } from "./const"
...
const Attributes = ({ title }: Props) => {
	const [status, setStatus] = useState<(typeof STATUS_DATA)[number] | null>(STATUS_DATA[1])
	const [assignees, setAssignees] = useState<typeof ASSIGN_DATA>([])
	const [priority, setPriority] = useState<(typeof PRIORITY_DATA)[number] | null>()
	const [tags, setTags] = useState<typeof TAGS_DATA>([])
	const [projects, setProjects] = useState<(typeof PROJECTS_DATA)[number] | null>(null)
			...
			<div className="pb-[18px] pt-1 px-6 flex gap-2">
				...
				<MultiSelectDropdown
					...
					onChange={(values) => {
						...
						setAssignees(ASSIGN_DATA.filter((item) => values.includes(item.value)))
					}}
				/>
				...
				<SelectDropdown
					...
					onChange={(value) => {
						setPriority(PRIORITY_DATA.find((item) => item.value === value) || null)
					}}
				/>
				...
				<SelectDropdown
					...
					onChange={(value) => {
						setProjects(PROJECTS_DATA.find((item) => item.value === value) || null)
					}}
				/>
			</div>
		</>
